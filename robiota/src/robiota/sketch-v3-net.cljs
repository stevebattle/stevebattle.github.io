...
(defn eye [b label a]
          (let [e (create-eye b a)]
            (aset e "userData" {:type "eye" :label label :joint (weld b e) }) ;; #js
            { :body e }))
...
(defn motor [b label a v]
  (let [m (create-motor b a v)]
    (aset m "userData" {:type "motor" :label label :joint (weld b m)}  ) ;; #js
    { :body m } ))
...
(defn touch [b label a]
  (let [t (create-touch b a)]
    (aset t "userData" {:type "touch" :label label :joint (weld b t) }) ;; #js
    { :body t }))

(defn net [ b label input output weight ]
  (let [n (js/world.CreateBody (js/b2BodyDef.)) ] ;; a neuron is a body with no fixture
      (aset n "userData"  {:label label :type "net" :input input :output output :weight weight}) ;; #js
      { :body n :joint b }) ;; joint can be simply the joined body
)
...
(defn ball []
  (let [shape (js/b2CircleShape.)
    ...
    (let  [ body (js/world.CreateBody bdef)] 
         ...
         (.CreateFixture body shape DENSITY)
         (aset body "userData" {:type "ball"}) ;;#js
         {:body body})))
  ...
  (loop [i 0]
    (when (< i VEHICLES)
      (vswap! parts concat (let [ pi Math/PI] ;; can't unquote constant?
        (cell "CELL"
          `(net "L1" "RIGHT_EYE" "LEFT_MOTOR" 1.0)
          `(net "R1" "LEFT_EYE" "RIGHT_MOTOR" 1.0)
          `(net "L2" "TOUCH" "LEFT_MOTOR" ~REVERSE)
          `(net "R2" "TOUCH" "RIGHT_MOTOR" ~REVERSE)
...
;;   (if (.hasOwnProperty u key)
;;     (let [v (aget u key)] 
;;       (if (coll? v) 
;;         (let [ sum (reduce + (map val (seq v))) ] sum ) 
;;         v
;;     ))
;;     0
;;   )
...
(defn solveTouch [p]
  (let [ t (:body p)
      ...
      ;;body (.GetBodyA (:joint p))
      body (.GetBodyA (getParameter t :joint))
      test (some? (some #{id} c))
      label (getValue t :label)
      value (if (some? (some #{id} c)) 1 0 )
    ]
    (setValue body label value)
))
...
(defn solveNet [p] 
  (let [ n (:body p) 
         ...
         t (getValue body input)
      ]
    (setMultiValue body output label (* t weight))
))
...
(defn step [] 
  (let [p (deref parts)]
   (loop [i 0]
     (when (< i (count p))
       (let [ type ((aget (:body (nth p i)) "userData") :type) ] ;;aget type
       (case type
...
(defn beginContact [contact]
  (let [ a (.GetBody (.GetFixtureA contact))
        ...
        aid (aget a "a")
        bid (aget b "a")
        atype ((aget a "userData") :type)
        btype ((aget b "userData") :type)
        c (deref contacts)
      ]
...
(defn removeFirst [x l]
  ...
  (concat n (rest m))) ;; add touch sensor to list (possibly multiple contacts for the same sensor)
)

...
(def CENTERING 0.05)
(def REVERSE -50)
(def SCALE 25) ;; for normal function
...
(defn radial [b label input bias]
  (let [n (js/world.CreateBody (js/b2BodyDef.))] ;; a neuron is a body with no fixture
    (aset n "userData"  {:label label :type "radial" :input input :bias bias :joint b}) n)) ;; joint is simply the joined body
  ...
  (loop [i 0]
    (when (< i VEHICLES)
      (vswap! parts concat (let [ pi Math/PI] ;; can't unquote constant?
                            (cell "CELL"
                                  `(net "L1" "RIGHT_EYE" "LEFT_MOTOR" 0.0)
                                  `(net "R1" "LEFT_EYE" "RIGHT_MOTOR" 0.0)
                                  `(net "L2" "TOUCH" "LEFT_MOTOR" 1.0)
                                  `(net "R2" "TOUCH" "RIGHT_MOTOR" 1.0)
                                  `(radial "RAD1" "CELL" 0) ;; what's the initial angle of the cell?
                            ;;      `(sigmoid "SIG1" [ "RAD1" ] 1 SLOPE)
                                  `(eye "RIGHT_EYE" 0) ;; labels needed for message passing
                                  `(eye "LEFT_EYE" ~(/ pi 3))
                                  `(touch "TOUCH" ~{"cell" REVERSE "motor" REVERSE "eye" REVERSE "ball" 20} ~(/ pi 6))
...
(defn solveRadial [n]
  (let [body (getParameter n :joint)
        ...
        v (getValue body input)
        r (normal (wrapAngle (- v bias)) SCALE)]

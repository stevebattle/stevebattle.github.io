...
(defn eye [b label a] ;; offset from motor body
...
(defn motor [b label a v] ;; a: angle from body, v: angle of thrust vector
  (let [rot (/ Math/PI 6)
...
(defn body [label & parts]
  (let [ b (createBody)
        blist (map (fn [p] (evaluate (cons (first p) (cons b (rest p))))) parts)  
        ]
    (doall (map (fn [bi] (vswap! joints conj (weld b bi))) blist)) 
    (aset b "userData" #js {:body label :LEFT_EYE 0 :RIGHT_EYE 0 :LEFT_MOTOR 0 :RIGHT_MOTOR 0}) ;; JS dictionary
    (cons b blist)
    ))
...
(defn setup []
  ...
  (loop [i 0]
    (when (< i VEHICLES)
      (vswap! bodies concat (let [ pi Math/PI] ;; can't unquote constant?
                              (body "BODY"
                                    `(eye "RIGHT_EYE" 0)
                                    `(eye "LEFT_EYE" ~(/ pi 3))
